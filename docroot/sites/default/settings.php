...
$databases = [];
$settings['hash_salt'] = 'V8xOY4-SWuaorodSBuqF5BGURfke1SflinqMYccR7NKuDeB4EckB20kf52OhvqN_gp4GtR51iA';
$settings['update_free_access'] = FALSE;
...
/**
 * Set $is_acquia to true if we detect Acquia environment variables.
 */
$is_acquia = isset($_ENV['AH_SITE_GROUP']) && isset ($_ENV['AH_SITE_ENVIRONMENT']);

/**
 * Connect to the Acquia database and set up Memcache
 */
if ($is_acquia && file_exists('/var/www/site-php')) {
 ...
 */
$env = $is_acquia ? $_ENV['AH_SITE_ENVIRONMENT'] : 'local';
...
if ($is_acquia) {
  ...
  if (file_exists($base_path . '/secrets.settings.local.php')) {
    require $base_path . '/secrets.settings.local.php';
  }
}
...
/**
 * The configuration export directory has to be declared after Acquia's
 * *-settings.inc and DDEV's config does it for us.
 */
 ...
 */
if (file_exists($base_path . '/settings.' . $env . '.php')) {
  require $base_path . '/settings.' . $env . '.php';
}
